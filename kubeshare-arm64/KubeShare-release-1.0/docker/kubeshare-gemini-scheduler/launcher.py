...
def update_podmanager(file):
    ...
    for i in range(1, podnum+1):
        ...
        if name_port not in podlist:
            sys.stderr.write("{} [launcher] pod manager id '{}' port '{}' start running\n".format(get_cur_time_str(), name_port, port))
            log_zevent('APP_POD_CREATED')
            sys.stderr.flush()
            proc = sp.Popen(
...
def main():
    ...
    # DEFINE ARGS BELOW
    parser.add_argument('--base_quota', type=float, default=300, help='base quota (ms)') # Default: 300
    parser.add_argument('--min_quota', type=float, default=20, help='minimum quota (ms)') # Default: 20
    parser.add_argument('--window', type=float, default=10000, help='time window (ms)') # Default: 10000
