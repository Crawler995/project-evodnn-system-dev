...
CUresult cuLaunchKernel_prehook(CUfunction f, unsigned int gridDimX, unsigned int gridDimY,
  ...
  // hDEBUG(log_name, __FILE__, (long)__LINE__, "cuLaunchKernel_prehook start, before request QUOTA");

  // original:
  if (!burst_predictor.ongoing_unmerged() &&
      us_since(request_start) / 1e3 + burst_predictor.predict_unmerged() >= quota_time) {
  
  // if (us_since(request_start) / 1e3 + burst_predictor.predict_unmerged() >= quota_time) { // trial
    ...
    hDEBUG(log_name, __FILE__, (long)__LINE__, "requesting QUOTA...");

    new_quota = get_token_from_scheduler(next_burst);

    hDEBUG(log_name, __FILE__, (long)__LINE__, "requesting QUOTA finished: %.3f", new_quota);
